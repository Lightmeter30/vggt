...
class EurocDataset(BaseDataset):
    def __init__(
        ...
        The heavy preprocessing steps (sequence discovery, GT alignment, calibration
        parsing, and IMU export) are expected to be handled offline by
        training/data/preprocess/generate_local_euroc_annotations.py.
        """
        ...
        if EUROC_DIR is None or EUROC_ANNOTATION_DIR is None:
            raise ValueError("Both EUROC_DIR and EUROC_ANNOTATION_DIR must be specified.")

        self.EUROC_DIR = osp.abspath(EUROC_DIR)
        self.EUROC_ANNOTATION_DIR = osp.abspath(EUROC_ANNOTATION_DIR)
        ...
        if not osp.isdir(self.EUROC_DIR):
            raise ValueError(f"EUROC_DIR does not exist: {self.EUROC_DIR}")
        if not osp.isdir(self.EUROC_ANNOTATION_DIR):
            raise ValueError(
                f"EUROC_ANNOTATION_DIR does not exist: {self.EUROC_ANNOTATION_DIR}"
            )
        ...
        annotation = self._load_split_annotation(split)
        ...
        for sequence_name, raw_sequence_data in sorted(annotation.items()):
            ...
            if frame_count < self.min_num_images:
                logging.info(
                    "Skipping EuRoC sequence %s: only %d matched frames",
                    sequence_name,
                    frame_count,
                )
                continue
        ...
        logging.info("%s: EuRoC sequence count: %d", status, self.sequence_list_len)
    ...
    def _load_split_annotation(self, split: str) -> Dict[str, Dict]:
        ...
        if not osp.isfile(annotation_file):
            raise FileNotFoundError(
                f"EuRoC annotation file not found: {annotation_file}. "
                "Run training/data/preprocess/generate_local_euroc_annotations.py first."
            )
    ...
    def get_data(
        ...
        if self.sequence_list_len == 0:
            raise RuntimeError("No valid EuRoC sequences were loaded.")
        ...
        for image_idx in ids:
            ...
            image_path = osp.join(self.EUROC_DIR, frame["image_rel_path"])
            image = read_image_cv2(image_path)
            if image is None:
                logging.warning("Failed to read EuRoC image: %s", image_path)
                continue
