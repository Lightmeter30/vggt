...
class Rectifier():
    ...
    def __init__(self):
        ...
        self.tumvi_dir = Path(os.path.abspath(self.args.dataset_path))
        ...
        self.orig_dir =  self.tumvi_dir / f"original"
        self.rect_dir =  self.tumvi_dir / f"rectified"
        sequences = os.listdir(self.orig_dir)
        sequences.sort()
        
        for seq in sequences:
            print("=" * 20)
            print(f"[+]Rectification starting for sequence {seq}.")
            self.seq = seq
            self.dataset_dir = self.tumvi_dir / f"original/{seq}"
            self.init_paths()
            self.load_yaml()
            self.calc_new_intrinsics()

            self.rectify(self.rect_dir / f"{seq}")

            if self.args.basalt_path:
                self.store_basalt_calib(Path(self.args.basalt_path), self.rect_dir / f"{seq}/basalt_calib.json")
            print()
    ...
    def rectify(self, rect_dirname):
        ...
        print(f"[+] Rectification of dataset sequence {self.seq} complete.")
    ...
    def store_basalt_calib(self, basalt_dir, new_calib_path):
